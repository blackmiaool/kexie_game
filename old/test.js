...
window.Zepto = Zepto, "$" in window || (window.$ = Zepto),
    ...
    var mm_wrap = Zepto(".msg-gift .boarder>ul");
    if (!mm_wrap.length) {
        alert("官方更新，脚本失效。");

    } else {
        if(typeof(mm_gift_scan)!="undefined")
            clearInterval(mm_gift_scan)
        else
            mm_gift_scan=0;
        var mm_li = []
        var mm_pre=[];
        function get_name($dom){
            return $dom.find(".user-name").text();
        }
        function get_gift($dom){
            return $dom.find(".user-name").next().next().attr("src");
        }
        function gift_is_star($dom){
            return $dom.html().lastIndexOf("小星星")!=-1;
        }
        var gift_num_reg=/[0-9]+(?=[^0-9]*$)/;
        function get_gift_num($dom){
            
            return gift_num_reg.exec($dom.html())[0];
        }
        function set_gift_num($dom,num){
            console.log("set  ",$dom,num);
            $dom.html($dom.html().replace(gift_num_reg,num));
            console.log($dom);
        }
        mm_gift_scan = setInterval(function () {
            if (!mm_pre.length) {
                console.log('missing')
                mm_li = mm_wrap.find("li").last();
                mm_pre=mm_li;
            }else{
                mm_li=mm_pre.next();
                while (mm_li.length) {
                    console.log("handle")
                    console.log("miao", mm_li);
                    var name = get_name(mm_li);
                    var gift=get_gift(mm_li);
                    if(name==get_name(mm_pre)&&gift==get_gift(mm_pre)){
                        console.log("get")
                        set_gift_num(mm_pre,get_gift_num(mm_li));
                        console.log("set");
                        mm_li[0].outerHTML="";
                        mm_li=mm_pre;
                    }else{
