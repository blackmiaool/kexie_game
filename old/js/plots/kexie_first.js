...
th(a, pp.daijun, b);
tc("那么，今天基本就这样吧，等待我们的下一步通知吧。",pp.daijun);
th(0, 0, 0);
tc("参观完科协，你带着复杂的心情回了宿舍。");
...
tc("十天后，你收到了科协的开会通知");
ts(img.jiaoshi);
tc("你来到通知中提到的教室，等待学长们的到来");
tc("20分钟后  戴俊带着其他几个学长来到教室，几个学长都一脸严肃，教室里顿时变得安静了。");
th(pp.tanjinchuan, pp.daijun, pp.chenguo);
tc("根据我们的探子得到的消息，总协学院明天要举办第22届EE杯了",pp.daijun);
tc("不错，EE杯分为大一大二大三三个组别，通常在学年初期举办。各大科协为了第一时间彰显自己科协的实力，吸引更多优秀人才，往往会对EE杯投入巨大的人力和物力",pp.chenguo);
tc("虽然EE杯是总协举办的，但是总协并不会对总协自己的参赛队伍有任何偏袒",pp.daijun);
tc("你们都知道的，我们只是一个小科协，所以之前在EE杯上取得的成绩并不突出，很少有人可以在EE杯中拿到一等奖",pp.daijun);
tc("毕竟我们实力有限，所以不会说什么一定要拿到一等奖之类的话，你们每个队能拿到三等奖我们就很满足了",pp.daijun);
tc("没错，只要能拿到三等奖，除了总协学院给出的微不足道的奖品之外，我们还会专门给你们提供我们内部的特殊奖品",pp.tanjinchuan);
tc("而且，万一，我是说万一，你们有人拿到了一等奖，科协中的VIP位置就归你了。",pp.daijun);
tc("我靠！不是吧!不是说好VIP座位等我拿到飞卡三等奖之后给我吗?!",pp.tanjinchuan);
tc("淡定淡定，反正不大可能有人拿到EE杯一等的。。。",pp.daijun);
tc("额。。好吧",pp.tanjinchuan);
tc("那么今天的会就到这里，大家回去努力吧！",pp.daijun);
th(0, 0, 0);
