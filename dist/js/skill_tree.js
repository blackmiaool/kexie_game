...
define(["angular", "v", "dbg"], function (angular, v, dbg) {
    ...
    var skill_tree;
    angular.module('home_app').controller("tab_skill_controller", function ($scope) {
        if (dbg.imdeveloper) {
            return;
        }
        //        skill_tree={}
        ...
        function canUnlearn(talentID, clickLeftRight, maxTier) {
            ...
            if (clickLeftRight == 0) {
                ...
                if (theCurrentRank < maxRank) {
                    if (v.state.skill_point == 0) return false;
        ...
        function getTalentID(talentName) {
            ...
            for (theTalentID = 0; talent[theTalentID]; theTalentID++) {
                if (talent[theTalentID][1] == talentName) return theTalentID;
            }
        }
        ...
        function hasDependentTalentWithPoints(talentID) {
            ...
            if (talentID != 0) loopStart = talentID - 1;else loopStart = talentID;
            loopStop = treeStartStop[theTree];

            while (loopStart <= loopStop) {
                if (talent[loopStart][5] && talent[loopStart][5][0] == talentID && rankTop[loopStart][0] != 0) return true;
                loopStart++;
            }
            return false;
        }
        ...
        function canTurnGreen(totalPoints, tree, oldMaxTier) {
            ...
            if (tree == 0) i = 0;else i = treeStartStop[tree - 1] + 1;
        ...
        function checkRequiredTalent(talentID) {
            ...
            if (reqTalent = talent[talentID][5]) {
                ...
                while (reqTalent[index * 2] != undefined) {
                    ...
                    if (rankTop[reqTalentID][0] < reqTalentPoints) return false;
                    index++;
                }
            }
            return true;
        ...
        $scope.rankTopOnClick = function (talentID) {
            ...
            if (theCurrentRank < maxRank) {
                ...
                if (talent[talentID][5]) pointsTree[theTree]++;
        ...
        $scope.rankTopOnRightClick = function (talentID) {
            ...
            if (rankTop[talentID][0] > 0) {
                ...
                if (rankTop[talentID][0] == 0) {
                    ...
                    if (talent[talentID][5]) document.getElementById("arrowYellow" + talentID).style.visibility = "hidden";
            ...
            if (pointsTree[theTree] == 1) document.getElementById(tree[theTree] + 'tabPointsText').innerHTML = textPoint;else document.getElementById(tree[theTree] + 'tabPointsText').innerHTML = textPoints;
        ...
        while (talent[talentInsertID]) {
            minLevel[talentInsertID] = getMinLevel(talentInsertID); //fill in the minLevel array
            rankBottom[talentInsertID] = [1, rank[talentInsertID][0]];
            if (talent[talentInsertID][2] != 1) rankTop[talentInsertID] = [0, rank[talentInsertID][0], rank[talentInsertID][1]];else rankTop[talentInsertID] = [0, rank[talentInsertID][0]];
            talentInsertID++;
        }
            ...
            while (talent[i] && talent[i][0] == printTreeID && i != talent.length - 1) {
                ...
                for (verticalCounter = 1; verticalCounter <= vertical; verticalCounter++) {
                    ...
                    <td bgcolor = "#a5a5a5" width = "1"><div style = "width: 1px;"></div></td></tr><tr><td></td><td bgcolor = "#4f4f4f" height = "2"></td><td></td></tr></table></div></div></div>';
                            ...
                            if (rankTop[i][0] >= 1) massiveReplaceString += '<div id = "iconOverGreen' + i + '" style = "position: relative; z-index:' + ((5 - talent[i][4]) * 10 + 2) + ';"><div id = "iconOver"><img src = "' + mia_path + 'icon-over-green.gif" border = "0" /></div></div>';else massiveReplaceString += '<div id = "iconOverGreen' + i + '" style = "visibility:hidden; position: relative; z-index:' + ((5 - talent[i][4]) * 10 + 1) + ';"><div id = "iconOver"><img src = "' + mia_path + 'icon-over-green.gif" border = "0"></div></div>';

                            if (rankTop[i][0] == talent[i][2]) massiveReplaceString += '<div id = "iconOverYellow' + i + '" style = "position: relative; z-index:' + ((5 - talent[i][4]) * 10 + 4) + ';"><div id = "iconOver"><img src = "' + mia_path + 'icon-over-yellow.gif" border = "0"></div></div>';else massiveReplaceString += '<div id = "iconOverYellow' + i + '" style = "visibility:hidden; position: relative; z-index:' + ((5 - talent[i][4]) * 10 + 3) + ';"><div id = "iconOver"><img src = "' + mia_path + 'icon-over-yellow.gif" border = "0"></div></div>';

                            if (rankTop[i][0] == 0) massiveReplaceString += '<div id = "talentThumb' + i + '" style = "visibility: hidden; position: relative;"><div id = "wrapper49"><img src = "' + mia_path + imageName + '.jpg" border = "0" ' + iconSize + '></div></div>';else massiveReplaceString += '<div id = "talentThumb' + i + '" style = "position: relative"><div id = "wrapper49"><img src = "' + mia_path + imageName + '.jpg" border = "0" ' + iconSize + '></div></div>';
